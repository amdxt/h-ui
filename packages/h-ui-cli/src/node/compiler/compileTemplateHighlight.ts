...
export const replaceVersion = (s: string) =>
    s.replace(/\*\*\*.+\*\*\*/g, '').trim()
...
export function compileTable(md: string, titleRe: RegExp): string {
    ...
    md = md.slice(
        (tableHeadMatched.index as number) + tableHeadMatched[0].length
    )
    ...
    md = md.slice(
        0,
        (tableFootMatched.index as number) + tableFootMatched[0].length
    )
...
export function compileMD(
    ...
    const componentName = path.match(HL_COMPONENT_NAME_RE)
        ? path.match(HL_COMPONENT_NAME_RE)![1]
        : ''
    if (!componentName) {
        console.log('未获取到组件名称')
        return
    }

    const attributesTable = parseTable(
        compileTable(md, options.titleAttributes)
    )
...
export function compileLanguageMD(
    varletConfig: Required<VarletConfig>,
    options: TemplateHighlightCompilerOptions
) {
    ...
    compileDir(DOCS_COMPONENTS_DIR, webTypes, varletConfig, options)
