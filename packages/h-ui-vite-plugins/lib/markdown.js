...
const shared_1 = require("@varlet/shared");
function htmlWrapper(html) {
    ...
    const cardGroup = hGroup
        .map((fragment) => fragment.includes('<h3')
...
function extractComponents(source) {
    ...
    source = source.replace(vueRE, (_, p1) => {
        ...
        const partComponents = partImports === null || partImports === void 0 ? void 0 : partImports.map((importer) => {
            ...
            return `<${(0, shared_1.kebabCase)(component)} />`;
        });
        return partComponents
            ? `<div class="varlet-component-preview">${partComponents.join('\n')}</div>`
            : '';
    });
    return {
        imports,
...
function injectCodeExample(source) {
    ...
    return source.replace(codeRE, (str) => {
        ...
        const attr = flags.some((flag) => str.includes(flag))
            ? 'playground-ignore'
            : '';
...
function highlight(str, lang, style) {
    ...
    if (style) {
        link =
            '<link class="hljs-style" rel="stylesheet" href="' + style + '"/>';
    }
    ...
    return {
        ...
        handleHotUpdate(ctx) {
            return __awaiter(this, void 0, void 0, function* () {
                ...
                ctx.read = function () {
                    return __awaiter(this, void 0, void 0, function* () {
                        return markdownToVue(yield readSource(), options);
                    });
                };
            });
        },
